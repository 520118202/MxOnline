...
class CourseInfoView(LoginRequiredMixin, View):
    ...
    def get(self, requeset, course_id):
        course = Course.objects.get(id=int(course_id))
        # 查询用户是否已经学习了该课程
        user_courses = UserCourse.objects.filter(user=requeset.user, course=course)
        if not user_courses:
            # 如果没有学习该门课程就关联起来
            user_courses = UserCourse(user=requeset.user, course=course)
            user_courses.save()
        ...
        # 通过所有课程的id，找到所有的课程，按点击量取5个
        relate_courses = Course.objects.filter(id__in=course_ids).order_by('-click_nums')[:5]
        # 获取全部资源
        all_resources = CourseResource.objects.filter(course=course)
        return render(requeset, 'course-video.html', {'course': course,
...
class AddCommentsView(View):
    ...
    def post(self, request):
        if not request.user.is_authenticated:
            ...
            return HttpResponse('{"status": "fail", "msg": "用户登录"}', content_type='application/json')
        course_id = request.POST.get('course_id', 0)
        comments = request.POST.get('comments', '')
        if int(course_id) > 0 and comments:
            # 实例化一个course_comments对象
            course_comments = CourseResource()
            # 获取评论的是哪门课程
            course = Course.objects.get(id=int(course_id))
