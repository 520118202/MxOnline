...
class AddUserAskView(View):
    ...
    def post(self, request):
        ...
        if userask_form.is_valid():
            ...
            # 如果保存成功，返回json字符串，后买你content type是告诉浏览器返回的数据类型
...
class AddFavView(View):
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated:
            # 未登录时返回json提示未登录，跳转到登录页面时再ajax中做的
            return HttpResponse('{"status": "fail", "msg": "用户未登录"}', content_type='applications/json')
        ...
        if exist_record:
            ...
            if int(id) > 0 and int(type) > 0:
                ...
                if int(type) == 1:
                    ...
                    org = Course.objects.get(id=int(id))
                    org.fav_nums += 1
                    org.save()
                elif int(type) == 3:
                    teacher = Teacher.objects.get(id=int(id))
                ...
                return HttpResponse('{"status": "fail", "msg": "收藏出错"}', content_type='applications/json')
